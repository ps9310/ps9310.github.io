...
async function copyProjectDeepLink(feedKey, teamsAppId) {
  try {
    //temp textarea for copy to clipboard functionality
    let textarea = document.createElement("textarea");
    const encodedContext = encodeURIComponent(`{"subEntityId":"mywork://feedkey?feedkey=${feedKey}"}`);
    //form the deeplink                       
...
async function navigateToTabWithUrl(launchUrl, teamsAppId) {
  try {
    //set entityId
    let entityId = '';
    if(launchUrl.includes('mywork://')) {
      entityId = 'mywork';
    } else if(launchUrl.includes('qwalk://')) {
      entityId = 'qwalk';
    }

    //form the deeplink and execute
    if(entityId == ''){
      throw new Error('Invalid entityId');
    } else {
      const encodedContext = encodeURIComponent(`{"subEntityId":"${launchUrl}"}`);
    ...
    console.error('Failed to navigate to tab with Url: ', err);
